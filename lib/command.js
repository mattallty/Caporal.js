...
class Command extends GetterSetter {
  ...
  argument(synopsis, description, validator, defaultValue) {
    this._args.push(new Argument(synopsis, description, validator, defaultValue, this._program));
    return this;
  }
   ...
   */
  _getLongOptions() {
    return this._options.map(opt => opt.getLongCleanName());
  }
  ...
  _checkArgsRange(argsArr) {
    ...
    if (argsCount < range.min || argsCount> range.max) {
      throw new WrongNumberOfArgumentError(
        "Wrong number of argument(s) for command " + this.name() + `. Got ${argsCount} argument(s), expected between ${range.min} and ${range.max}`,
        {},
        this._program
      )
    }
  ...
  _camelCaseOptions(options) {
    return this._options.reduce((acc, opt) => {
      if (typeof options[opt.getLongCleanName()] !== 'undefined') {
        acc[opt.getName()] = options[opt.getLongCleanName()];
      }
      return acc;
    }, {});
  }
  ...
  option(synopsis, description, validator, defaultValue, required) {
    this._options.push(new Option(synopsis, description, validator, defaultValue, required, this._program));
    return this;
  }
...
Object.defineProperties(Command, {
  "NATIVE_OPTIONS": {
    value: ['h', 'help', 'V', 'version', 'no-color', 'quiet', 'silent', 'v', 'verbose']
  }
});
