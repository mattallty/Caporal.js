...
class Autocomplete {
  ...
  _getPossibleArgumentValues(currentCommand, lastPartialIsOption, argsCount) {
    if (!currentCommand ||
      lastPartialIsOption ||
      !currentCommand.args(argsCount)) {
      return Promise.resolve([]);
    }

    const arg = currentCommand.args(argsCount);

    // Choices
    if (arg.getChoices().length) {
      return Promise.resolve(arg.getChoices().map(choice => choice + ':Value for argument ' + arg.synopsis()));
    }
  ...
  _complete(data, done) {
    ...
    const possArgValues = this._getPossibleArgumentValues(currCommand, lastPartIsOption, realArgsCount);
